...
World.prototype.removeTownById = function(id) {
	this.towns.splice(id, 1);
};
...
World.prototype.removeCitizenById = function (id) {
...
World.prototype.generateCitizens = function(min, max) {
	...
	for (i; i < end; ++i ) {
		var citizen = new Citizen(getRandomCitizenName('male'), getRandomCitizenSurname(), getRandomInt(0,100));
		this.citizens[citizen.id] = citizen;
	}
};
...
World.prototype.generateTowns = function(min, max) {
	...
	for (i; i < end; ++i ) {
		var randomCoords = world.getRandomCoords();
		var town = new Town(getRandomTownName(),randomCoords[0],randomCoords[1]);
		this.towns[town.id] = town;
		this.map[town.getX()][town.getY()] = town.id;
	}
};

World.prototype.getRandomCoords = function () { //TODO Do while not a good option
